  ...
  return new Promise<fabric.Image>((resolve) => {
    fabric.Image.fromURL(
      ...
      function (objectImage) {
        objectImage.on("loaded", function () {
          const effect = createFilter(filter);
          objectImage.filters = effect ? [effect] : [];
          objectImage.applyFilters();
          resolve(objectImage);
        });
      },
      {
...
export const downloadFile = (file: string, type: string) => {
  ...
  anchorElement.download = `mycanva-${uuid()}.${type}`;
  document.body.appendChild(anchorElement);
  anchorElement.click();
  anchorElement.remove();
};

export const transformText = (object: any) => {
   if (!object) return;

   object.forEach((element: any) => {
     if(element.object){
      transformText(element.object);
     }else{
